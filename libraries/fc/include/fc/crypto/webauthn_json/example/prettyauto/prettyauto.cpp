...
int main(int, char*[]) {
    ...
    OutputStream pico(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(pico);     // CHANGED
#else
    ...
    OutputStream pico(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(pico);             // CHANGED
#endif
